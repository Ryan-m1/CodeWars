...
上文我们探究了MyBatis执行SQL语句的流程 [原文链接](/blog/backend_developer/Mybatis/ExecuteSQL.md)，虽然当时仅仅只是提了一嘴缓存相关，但是却提供了一个入口点，那就是SqlSession#selectList()-》DefaultSqlSession#selectList()-》Executor#query()-》BaseExecutor#query()
...
在上文[原文链接](/blog/backend_developer/Mybatis/ExecuteSQL.md)分析SQL语句执行的过程中，我们了解了配置文件的加载和解析，其中缓存相关并没有去做说明，在本文会详细说明。
